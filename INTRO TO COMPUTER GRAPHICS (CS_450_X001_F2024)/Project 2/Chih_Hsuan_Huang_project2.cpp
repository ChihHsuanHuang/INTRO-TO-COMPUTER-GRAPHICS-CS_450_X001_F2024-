...
Animate()
{
	...
	RotationTime = (float)ms / (float)MS_IN_THE_ANIMATION_CYCLE;        // [ 0., 1. )
...
Display()
{
	...
	if (NextPerspective == OUTSIDE) {
		
		gluLookAt(0.f, 5.f, 10.f,   
			0.f, 0.f, 0.f,    
			0.f, 1.f, 0.f);   
	...
	glutWireTorus(0.5, 1.5, 16, 30);
	glPopMatrix();

	glPushMatrix();
	glTranslatef(0., 2.9, -2.0);   
	glRotatef(3600. * RotationTime, 0., 1., 0.);  
	glRotatef(90., 1., 0., 0.);
	...
	glRotatef(2. * 3600. * RotationTime, 1., 0., 0.);  
	glRotatef(90., 0., 1., 0.);
